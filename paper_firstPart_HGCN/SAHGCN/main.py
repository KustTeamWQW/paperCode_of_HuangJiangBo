...
val_ratio=0.001
#選擇模型
mode_name="paper_firstPart_HGCN"
# mode_name="GCN"
# mode_name="F2HGNN"
...
#要改
path_weight= r"D:\project\code\HGNN-base\paper_firstPart_HGCN\SAHGCN\weights"
path_result= r"D:\project\code\HGNN-base\paper_firstPart_HGCN\SAHGCN\results"
...
for i in range(Train_num):
    ...
    # model
    if mode_name=="paper_firstPart_HGCN":
        net=SAHGCN.HGCN( bands+2, 128,class_num, Q, A).to(args.device)
    elif mode_name=="GCN":
        net=GCN.GCN(bands,128,class_num,Q,A).to(args.device)
    elif mode_name=="F2HGNN":
        net=F2HGNN.HGNN_weight(net_input.shape[-1],256,class_num,Q,A,dropout=0.1).to(args.device)
...
# print("Train_time",np.mean(Train_Time_ALL, 0), '+-', np.std(Train_Time_ALL, 0))
