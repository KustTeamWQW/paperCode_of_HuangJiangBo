...
def construct_H_with_KNN_from_distance(dis_mat, k_neig, is_probH=False):
    ...
    return H
def construct_H_with_KNN_from_distance2(dis_mat, k_neig, is_probH=True, m_prob=1, sig=1000):
    """
    construct hypregraph incidence matrix from hypergraph node distance matrix
    :param dis_mat: node distance matrix
    :param k_neig: K nearest neighbor
...
def createH(image_spe,image_spa,segments):
    ...
    if usingkeypoint==1:
        K_a = 8
        n_nodes = s2D_whole_spe.shape[0]
        adj1 = np.zeros((n_nodes, n_nodes))
        for i in range(n_nodes):
            sorted_indices = np.squeeze(np.argsort(s2D_whole_spe[i]))
            sorted_indices=sorted_indices[0,:K_a]
            adj1[i, sorted_indices] = 1

        n_nodes = s2D_whole_spa.shape[0]
        adj2 = np.zeros((n_nodes, n_nodes))
        for i in range(n_nodes):
            sorted_indices = np.squeeze(np.argsort(s2D_whole_spa[i]))
            sorted_indices = sorted_indices[0, :K_a]
            adj2[i, sorted_indices] = 1
        adj=0.5*adj1+0.5*adj2
        # 创建一个图对象
        G = nx.Graph(adj)
        K=int(np.ceil(0.1*n_nodes))
        # 计算度中心性
        degree_centrality = nx.degree_centrality(G)
        # 选择度中心性最高的K个节点
        sorted_degree_centrality = sorted(degree_centrality.items(), key=lambda x: x[1], reverse=True)
        key_degree_nodes = [node for node, _ in sorted_degree_centrality[:K]]
        # 构造关联矩阵H
        N = n_nodes
        M1 = len(key_degree_nodes)
        H1 = np.zeros((N, M1), dtype=int)
        for i, node in enumerate(key_degree_nodes):
            # 找到与关键节点相邻的节点
        ...
        # key_betweenness_nodes = [node for node, _ in sorted_betweenness_centrality[:K]]
        # M2=len(key_betweenness_nodes)
        # H2 = np.zeros((N, M2), dtype=int)
        #
        # # 构建关联矩阵H2
        # for i, node in enumerate(key_betweenness_nodes):
        #     # 找到与关键节点相邻的节点
        #     neighbors = np.where(adj[node] > 0)[0]
        #     H2[neighbors, i] = 1
        #
        # H_key=np.concatenate((H1,H2),axis=1)
        H_key=H1
    if createWay==1:
    ...
    if createWay==2:
        H_whole_spe = construct_H_with_KNN_from_distance2(s2D_whole_spe, k_spe, sig=1000)
        H_whole_spa = construct_H_with_KNN_from_distance2(s2D_whole_spa, k_spa, sig=100)

    #PV
    H_whole=0.7*H_whole_spa+0.3*H_whole_spe
    #xuzhou
    # H_whole = np.concatenate((H_whole_spe, H_whole_spa), axis=1)
    # H_whole = np.concatenate((H_whole, H_key), axis=1)
