...
import torch
import time
from model import EHGCN,GCN,F2HGNN,utils,CNN
...
parser = argparse.ArgumentParser(description='paper_firstPart_HGCN')
...
path_weight=r"D:\project\code\HGNN-base\TGRS\SAHGCN\weights\\"
path_result= r"D:\project\code\HGNN-base\TGRS\SAHGCN\results\LaoYuHe\\"
...
#PV 0.05
    # model
    if mode_name=="advHGCN":
        net=EHGCN.EHGCN(height, width, bands+2, class_num, Q, A,alph=0.05).to(args.device)
    if mode_name== "HGCN":
            net = EHGCN.HGCN(bands, 128,class_num, Q, A).to(args.device)
    elif mode_name=="GCN":
        net=GCN.GCN(bands,128,class_num,Q,A).to(args.device)
    elif mode_name=="CNN":
        net=CNN.CNN2DModel(bands, 128, class_num).to(args.device)
    elif mode_name=="F2HGNN":
        net=F2HGNN.HGNN_weight(net_input.shape[-1],128,class_num,Q,A,dropout=0.1).to(args.device)
    ...
    Draw_Classification_Map(classification_map, "results\\LaoYuHe\\Image\\" +mode_name+"-"+ data_set_name + str(testOA)+"TGRS_label_ration"+str(train_ratio)+"superpixel_scale"+str(superpixel_scale))
    Draw_Classification_Map(masked_predicted_labels, "results\\LaoYuHe\\Mask\\" + mode_name+"-"+data_set_name + str(testOA)+"TGRS_label_ration"+str(train_ratio)+"superpixel_scale"+str(superpixel_scale))
    ...
    if save_results==1:
        ...
        f = open(path_result + run_date +mode_name+"-"+data_set_name +"TGRS"+str(train_ratio)+ '.txt', 'a+')
        str_results = '\n ======================' \
                    + '\nrun data = ' + run_date \
